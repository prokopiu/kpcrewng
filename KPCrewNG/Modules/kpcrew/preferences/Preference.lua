...
function kcPreference:render()
	...
	kc_imgui_out_text(color_white, splitTitle[1])
		
	if self.datatype == self.typeInt then
		kc_imgui_in_intfield(splitTitle[1], 0, color_orange, tonumber(splitTitle[2]), self:getValue(), function (textin) self:setValue(textin) end)
	end
	
	if self.datatype == self.typeFloat then
		kc_imgui_in_floatfield(splitTitle[1], 0, color_orange, tonumber(splitTitle[2]), splitTitle[3], self:getValue(), 
			function (textin) self:setValue(textin) end)
	end
--	
	if self.datatype == self.typeText then
		kc_imgui_in_tfield(splitTitle[1], 0, color_orange, 255, self:getValue(), function (textin) self:setValue(textin) end)
	end
--	
	...
	if self.datatype == self.typeInfo then
		...
		kc_imgui_out_text(splitTitle[2], self:getValue(), 330)
	end
--	
	if self.datatype == self.typeToggle then
		kc_imgui_in_rbtoggle(splitTitle[2], splitTitle[3], self:getValue(), function (textin) self:setValue(textin) end)
	end

	if self.datatype == self.typeList then
		kc_imgui_in_combolist(splitTitle[1], splitTitle, self:getValue(), function (textin) self:setValue(textin) end)
	end
--
